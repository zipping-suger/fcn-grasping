...
from trainer import TSTrainer, get_prediction_vis
...
def main(args):
    ...
    # ------------- Algorithm options -------------
    teacher_reward_discount = args.teacher_reward_discount
    experience_replay = args.experience_replay  # Use prioritized experience replay?

    # ------ Pre-loading and logging options ------
    load_snapshot = args.load_snapshot  # Load pre-trained snapshot of model?
    teacher_snapshot_file = args.teacher_snapshot_file  # Load teacher model
    ...
    # Initialize trainer
    trainer = TSTrainer(teacher_reward_discount, teacher_snapshot_file, load_snapshot,
                        student_snapshot_file=snapshot_file, force_cpu=force_cpu)
        ...
        while True:
            if nonlocal_variables['executing_action']:
                ...
                if save_visualizations:
                    student_grasp_1_pred_vis = get_prediction_vis(student_grasp_1_predictions, color_heightmap,
                                                                  nonlocal_variables['best_pix_ind'])
                    logger.save_visualizations(trainer.iteration, student_grasp_1_pred_vis, 's_grasp_1')
                    cv2.imwrite('visualization.s_grasp_1.png', student_grasp_1_pred_vis)
                    student_grasp_2_pred_vis = get_prediction_vis(student_grasp_2_predictions, color_heightmap,
                                                                  nonlocal_variables['best_pix_ind'])
                    logger.save_visualizations(trainer.iteration, student_grasp_2_pred_vis, 's_grasp_2')
                    cv2.imwrite('visualization.s_grasp_2.png', student_grasp_2_pred_vis)

                    teacher_grasp_1_pred_vis = get_prediction_vis(teacher_grasp_1_predictions, color_heightmap,
                                                                  nonlocal_variables['best_pix_ind'])
                    logger.save_visualizations(trainer.iteration, teacher_grasp_1_pred_vis, 't_grasp_1')
                    cv2.imwrite('visualization.t_grasp_1.png', teacher_grasp_1_pred_vis)
                    teacher_grasp_2_pred_vis = get_prediction_vis(teacher_grasp_2_predictions, color_heightmap,
                                                                  nonlocal_variables['best_pix_ind'])
                    logger.save_visualizations(trainer.iteration, teacher_grasp_2_pred_vis, 't_grasp_2')
                    cv2.imwrite('visualization.t_grasp_2.png', teacher_grasp_2_pred_vis)
        ...
        if 'prev_color_img' in locals():
            ...
            if experience_replay:
                ...
                if sample_primitive_action == 'grasp_1':
                    ...
                    sample_reward_value = 0 if real_value == 1 else 1
                ...
                if sample_ind.size > 0:
                    ...
                    with torch.no_grad():
                        sample_grasp_1_predictions, sample_grasp_2_predictions, sample_state_feat = trainer.ts_forward(
                            'student',
                    ...
                    trainer.backprop('student', sample_color_heightmap, sample_depth_heightmap, sample_best_pix_ind,
            ...
            # Save model snapshot
            logger.save_backup_model(trainer.student_model, 'student')
            logger.save_backup_model(trainer.teacher_model, 'teacher')

            if trainer.iteration % 100 == 0:
                logger.save_model(trainer.iteration, trainer.student_model, 'student')
                logger.save_model(trainer.iteration, trainer.teacher_model, 'teacher')

                if trainer.use_cuda:
                    trainer.student_model = trainer.student_model.cuda()
                    trainer.teacher_model = trainer.teacher_model.cuda()
...
if __name__ == '__main__':
    ...
    # ------------- Algorithm options -------------
    parser.add_argument('--teacher_reward_discount', dest='teacher_reward_discount', type=float, action='store',
                        default=1)
